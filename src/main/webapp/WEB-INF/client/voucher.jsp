    ...
    <div class="container">
        <div class="voucher-container">
            <c:if test="${empty vouchers}">
                <div style="padding: 80px"><h4 style="color: #8f8f8f">No vouchers available.</h4></div>
            </c:if>
            <c:forEach var="voucher" items="${vouchers}">
                <div class="voucher" data-voucher-id="${voucher.id}">
                    ...
                    <div class="voucher-header">
                        ...
                        <div>
                            <c:if test="${empty voucher.product and empty voucher.category}">
                                <div>ALL</div>
                            </c:if>
                    ...
                    <div class="voucher-details">
                        <div><c:out value="${voucher.code}"/></div>
                    ...
                    <div class="voucher-action">
                        <c:choose>
                            <c:when test="${voucher.quantity <= 0 and empty sessionScope.account}">
                                <span class="soldout-button">Hết lượt sử dụng</span>
                            </c:when>
...
<script>
    function saveVoucher(voucherId, element) {
        $.ajax({
            url: 'SaveVoucherControll',
            method: 'POST',
            data: {voucherId: voucherId},
            success: function (response) {
                alert('Lưu voucher thành công');
                $(element).text('Đã lưu');
                $(element).addClass('saved-button');
                $(element).removeClass('save-button');
                $(element).attr('onclick', 'return false;'); // Disable click event
            },
            error: function () {
